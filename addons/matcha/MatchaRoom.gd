...
# Private methods
func __poll():
	if get_connection_status() == CONNECTION_DISCONNECTED:
		return
	_create_offers()
...
func _create_offer() -> void:
	...
	var err := add_peer(offer_peer, offer_rpc_id)
	if err != OK:
		return
...
func close() -> void:
	if Engine.get_main_loop().process_frame.is_connected(self.__poll):
		Engine.get_main_loop().process_frame.disconnect(self.__poll)
	
	for tracker_client in _tracker_clients:
		if tracker_client.has_method("close"):
			tracker_client.close()
	_tracker_clients.clear()
	
	# Calls WebRTCMultiplayerPeer.close() to shut down WebRTC connections
	super.close()
...
func _on_got_offer(offer: TrackerClient.Response, tracker_client: TrackerClient) -> void:
	# Guard against processing offers when disconnected or closed
	if get_connection_status() == CONNECTION_DISCONNECTED:
		return
	if offer.info_hash != _room_id: return
	if find_peer({ "peer_id": offer.peer_id }) != null: return
	if _type == "client" and offer.peer_id != room_id: return
...
func _on_got_answer(answer: TrackerClient.Response, tracker_client: TrackerClient) -> void:
	# Guard against processing answers when disconnected or closed
	if get_connection_status() == CONNECTION_DISCONNECTED:
		return
	if answer.info_hash != _room_id: return
	if _type == "client" and answer.peer_id != room_id: return

	var offer_peer: MatchaPeer
	if _type == "client":
		if has_peer(1):
			offer_peer = get_peer(1).connection
			offer_peer.set_offer_id(answer.offer_id)
	else:
		offer_peer = find_peer({ "offer_id": answer.offer_id })
	if offer_peer == null: return
